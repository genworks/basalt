...
;;; register; glossary.sexp beside it maps these keys (and the role
;;; names) onto the ones the generator reads natively.  Only the
;;; generator's outputs speak docker and compose.
;;;
...
(
 ...
 (:restart "unless-stopped"
  ...
  :timezone "${TZ:-Etc/UTC}"
  :network-ipv6 t
  ;; Deviation from the upstream defaults: a Basalt deployment can run
  ;; beside an upstream one on the same host without the two networks
  ;; contending for one address pool.
 ...
 ;; know.  A service's :role may be a LIST -- one service filling
 ...
 ;;
 ;; The :requires strings are matched EXACTLY against each image's
