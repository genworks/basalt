...
- `rmacs` and `grmacs` become shell commands that connect you to
...
stack built with the same tooling, say) need no instance name, since
each clone is its own compose project; they need only distinct
published ports. The offset is read from the
...
Everything beyond the standard set, such as licensed engine variants,
an ingress (reverse proxy) or a monitoring dashboard, arrives by
**stack repository**: a repository beside this one, one per deployment
...
Configuration that must name other services, such as an ingress's
routing rules or a services-init hook, names a **role and a port**:
...
Copyright © 2026 Gornskew Enterprises. Basalt modifications
